#!/usr/bin/env python3
"""Enrich arts_question definitions only."""
...
print(f"Arts: {len(rows)} to enrich")
...
for i in range(0, len(rows), 5):
    ...
    prompt = f"以下の芸術の概念・運動に対して、学術的に正確な定義文を生成してください。各定義は2-3文。\n\n{clist}\n\nJSON配列のみ出力:\n[{{\"name_en\":\"name\",\"definition\":\"定義文\"}}]"
    try:
...
conn.close()
print(f"Arts DONE: {updated} updated")

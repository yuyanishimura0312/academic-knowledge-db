#!/usr/bin/env python3
"""Enrich natural_discovery definitions only."""
...
print(f"Natural sciences: {len(rows)} to enrich")
...
for i in range(0, len(rows), 5):
    ...
    prompt = f"以下の自然科学の学術概念に対して、学術的に正確な定義文を生成してください。各定義は2-3文。\n\n{clist}\n\nJSON配列のみ出力:\n[{{\"name_en\":\"name\",\"definition\":\"定義文\"}}]"
    try:
...
conn.close()
print(f"Natural sciences DONE: {updated} updated")

#!/usr/bin/env python3
"""Import Phase 5 poetics final push (5 Codex workers) into academic.db."""

import json, sqlite3, uuid
from pathlib import Path

DB_PATH = Path(__file__).parent / "academic.db"
COLLECT_DIR = Path("/tmp/poetics_collect_p5")
FILES = sorted(f.name for f in COLLECT_DIR.glob("P5_*.json"))
...
def main():
    ...
    for sf, n in cur.fetchall():
        ...
        total += n
    print(f"  TOTAL: {total}")
    print(f"  Coverage: {total}/1155 = {total/1155*100:.1f}%")

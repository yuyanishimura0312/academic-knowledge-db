#!/usr/bin/env python3
"""Phase 9 Step 2: 薄サブフィールド拡張 - 16タスク、各60件目標。"""
...
OUT_DIR = ROOT / "p9_step2_outputs"
LOG_DIR = ROOT / "p9_step2_logs"
...
PROMPT = """「{subfield}」分野の重要概念60件を JSON 配列で出力。
...
要件: 内部知識のみ、subfield統一、60件、JSON配列のみ（説明なし）。"""
...
def run_one(task: dict) -> tuple[str, int, int]:
    ...
    print(f"[p9s2] {tid} ({task['subfield']}) start at {time.strftime('%H:%M:%S')}", flush=True)
    try:
        with open(out, "wb") as fout, open(log, "wb") as flog:
            rc = subprocess.run(
                ["codex", "exec", "--full-auto", prompt],
                stdout=fout, stderr=flog, timeout=300,
            ).returncode
    except subprocess.TimeoutExpired:
        rc = 124
    sz = out.stat().st_size if out.exists() else 0
    print(f"[p9s2] {tid} done rc={rc} bytes={sz}", flush=True)
    return tid, rc, sz


def main() -> int:
    print(f"[p9s2] {len(TASKS)} tasks, MAX_PARALLEL={MAX_PARALLEL}", flush=True)
    results = []
    with ThreadPoolExecutor(max_workers=MAX_PARALLEL) as ex:
        ...
        for fut in as_completed(futures):
            try:
                ...
                print(f"[p9s2] task raised: {e}", flush=True)

...
OUT_DIR = ROOT / "p9_step3_outputs"
LOG_DIR = ROOT / "p9_step3_logs"
OUT_DIR.mkdir(exist_ok=True)
LOG_DIR.mkdir(exist_ok=True)
TASKS = json.loads((ROOT / "p9_step3_tasks.json").read_text(encoding="utf-8"))["tasks"]
MAX_PARALLEL = 6
...
各エントリ:
{{"name_ja":"日本語","name_en":"English","definition":"100字以上","subfield":"{subfield}","school_of_thought":"学派/枠組","era_start":年(2015以上),"era_end":null,"keywords_en":"k1,k2","key_researchers":["名前"]}}
...
def run_one(task):
    ...
    print(f"[p9s3] {tid} start at {time.strftime('%H:%M:%S')}", flush=True)
    try:
    ...
    print(f"[p9s3] {tid} done rc={rc} bytes={sz}", flush=True)
    return tid, rc, sz


def main():
    print(f"[p9s3] {len(TASKS)} tasks, MAX_PARALLEL={MAX_PARALLEL}", flush=True)
    results = []

#!/usr/bin/env python3
"""Phase 12 Step 1: humanities + engineering 14並列拡張、各60件目標。"""
...
OUT_DIR = ROOT / "p12_outputs"
LOG_DIR = ROOT / "p12_logs"
OUT_DIR.mkdir(exist_ok=True)
LOG_DIR.mkdir(exist_ok=True)
TASKS = json.loads((ROOT / "p12_tasks.json").read_text(encoding="utf-8"))["tasks"]
MAX_PARALLEL = 6
...
def run_one(task):
    ...
    print(f"[p12] {tid} start at {time.strftime('%H:%M:%S')}", flush=True)
    try:
    ...
    print(f"[p12] {tid} done rc={rc} bytes={sz}", flush=True)
    return tid, rc, sz


def main():
    print(f"[p12] {len(TASKS)} tasks", flush=True)
    results = []
